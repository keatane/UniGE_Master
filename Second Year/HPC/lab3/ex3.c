...
int nums = omp_get_max_threads();

double* arr = (double*)calloc(sizeof(double),nums);
for (i=0; i<nums; i++)
    arr[i] = 0;

#pragma omp parallel for private(x,f,sum)
    for (i = 1; i <= intervals; i++) {
        x = dx * ((double) (i - 0.5));
        f = 4.0 / (1.0 + x*x);
        sum = sum + f;
        arr[omp_get_thread_num()] += f;
    }
    printf("Taken: %lf\n", omp_get_wtime() - start); 
    
    for (i=0; i<nums; i++) sum += arr[i];
    free(arr);
    pi = dx*sum;
